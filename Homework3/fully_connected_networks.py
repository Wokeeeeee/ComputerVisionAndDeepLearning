...
class Linear(object):
  ...
  def forward(x, w, b):
    ...
    out=torch.mm(transform,w)+b  #x*w+b
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
    cache = (x, w, b)
  ...
  def backward(dout, cache):
    ...
    #############################################################################
    # Replace "pass" statement with your code
    # dout是上流梯度：dL/dy   L是某个损失函数
    # 反向传播算法的矩阵维度分析详见：https://zhuanlan.zhihu.com/p/29611333
    N=x.shape[0]
    dx_ori=torch.mm(dout,w.t())
    dx=dx_ori.reshape(x.shape)
